...
// Import FontLoader which is needed for neon signs
...
function initGame() {
    ...
    console.log("Initializing game...");
        ...
        // Create X-Machina Hub
        const { hubGroup, edgeRing, innerRing, topSphere, beamMaterial, sphereGlow } = createXMachinaHub();
        scene.add(hubGroup);
        animateXMachinaHub(hubGroup, edgeRing, innerRing, topSphere, beamMaterial, sphereGlow);
        
        // Add Cyborg Girl character near the hub
        const hubPosition = hubGroup.position;
        window.cyborgGirl = addCyborgGirlToGame(scene, hubPosition.x + 20, 1, hubPosition.z + 10);
        window.cyborgGirl.rotation.y = -Math.PI / 4; // Rotate to face toward center/player
...
function createNeonSigns() {
    ...
    // Exclude the tallest building from further sign placement
    // Filter out the tallest building to avoid placing additional signs on it
    const otherTallBuildings = tallBuildings.filter(b => b !== tallestBuilding);
    
    // Limit the number of neon signs to avoid performance issues
    const maxSigns = Math.min(8, otherTallBuildings.length);
    
    // Randomly select buildings for neon signs (excluding the MATRIX building)
    ...
    console.log(`Created neon signs on ${selectedBuildings.length + 1} buildings (including exclusive MATRIX sign)`);
}
...
function createCityscape() {
    ...
    for (let i = 0; i < BUILDING_COUNT; i++) {
        ...
        // Try to find a position that doesn't intersect with the energy conduit
        while (intersectsConduit && attempts < 50) {
        ...
        if (intersectsConduit) {
            console.log("Couldn't place building without conduit intersection after max attempts, skipping");
            continue;
        }
        ...
        // Choose a color (shades of blue and cyan)
        ...
        // PBR building material (dark cyan/blue with glass-like properties)
        ...
        // Create window pattern material with PBR
        const windowsMaterial = new THREE.MeshStandardMaterial({
            color: new THREE.Color(`hsl(${hue}, ${saturation}%, ${lightness + 50}%)`),
            emissive: new THREE.Color(`hsl(${hue}, ${saturation}%, ${lightness + 50}%)`),
        ...
        // Building as a group to add details
        ...
        // Add random rotation for some buildings (not all)
        if (Math.random() > 0.7) {
    ...
    console.log("Cityscape creation complete. Actual building count:", buildings.length);
}
...
// Create game screens (start, pause, game over)
function createGameScreens() {
    ...
    startScreen.innerHTML = `
        <h1>X-Machina: Aurora Prime Explorer</h1>
        <p>Collect data fragments while avoiding enemy drones and buildings.</p>
    ...
    document.getElementById('start-button').addEventListener('click', () => setGameState('playing'));
...
function setupEventListeners() {
    ...
    document.addEventListener('keydown', function(event) {
        switch(event.key) {
            ...
            case ' ':
                ...
                break;
            case 'f':
                // F key - fire projectile (only process if not in cooldown)
                if (!weaponCooldown && Date.now() - lastFireTime >= WEAPON_COOLDOWN_TIME) {
                    fireProjectile();
                }
                break;
            case 'Escape':
            ...
            case 'b':
                ...
                break;
            case 'q':
                // Toggle spaceship quality test panel
                const existingPanel = document.getElementById('spaceship-test-panel');
                if (existingPanel) {
                ...
                break;
            case 'Shift':
                // While shift is held, lower the ship
                moveDown = true;
                break;
            case 'Control':
                // While control is held, lower the ship
                moveDown = true;
                break;
        }
    });
    ...
    // Add keyboard event listener for quality testing
    document.addEventListener('keydown', function(event) {
        // Press 'Q' to toggle quality test panel
...
function completeLevel() {
    ...
    const crypticMessage = `
        <div style="font-family: monospace; letter-spacing: 2px; line-height: 1.5;">
            <p style="color: #ff0066; text-shadow: 0 0 5px #ff0066;">E#R0R_932: SYSTEM BREACH</p>
            <p style="color: #0ff; text-shadow: 0 0 8px #0ff;">D4T4_FR4GM3NT5_C0MPR0M153D</p>
            <p style="color: #ffcc00; text-shadow: 0 0 3px #ffcc00;">UNKN0WN_3NT1TY_D3T3CT3D</p>
            <p style="color: #ff00ff; text-shadow: 0 0 7px #ff00ff; font-size: 0.8em; margin-top: 10px;">01001100 01001111 01010011 01010100</p>
        </div>
    `;
    ...
    levelCompleteDiv.innerHTML = `
        ...
        ${crypticMessage}
        <button id="continue-button" style="background: #0ff; color: #000; border: none; padding: 10px 20px; margin-top: 20px; font-family: monospace; cursor: pointer; font-size: 18px;">ACKNOWLEDGE</button>
    `;
...
function createDrones() {
    ...
    for (let i = 0; i < dronePool.length && dronesCreated < dronesNeeded; i++) {
        ...
        console.log("Activated drone:", droneData.id, "at position:", droneData.position.x.toFixed(2), droneData.position.y.toFixed(2), droneData.position.z.toFixed(2));
    }
...
function animate() {
    ...
    // Animate cyborg girl character if present
    if (window.gameCharacters && window.gameCharacters.length > 0) {
        window.gameCharacters.forEach(char => {
            if (char && char.animate) {
                char.animate(deltaTime);
            }
        });
    }
...
function updateIndicators() {
    ...
    // Check proximity to cyborg girl
    checkCyborgGirlProximity();
}
...
// Create the X-MACHINA HUB - AI system controlling AuraPrime
function createXMachinaHub() {
    // Position at the western edge of the X-axis
    ...
    // Base platform (red circular platform with more detailed geometry)
    const basePlatformGeometry = new THREE.CylinderGeometry(40, 42, 3, 64, 3);
    ...
    basePlatform.position.y = -0.2; // Slightly above ground
    basePlatform.receiveShadow = true;
    hubGroup.add(basePlatform);
    
    // Add enhanced glowing edge ring to base platform
    const edgeGeometry = new THREE.TorusGeometry(40, 0.8, 24, 128);
    ...
    edgeRing.rotation.x = Math.PI / 2; // Lay flat
    edgeRing.position.y = 1.0; // Just above the platform
    hubGroup.add(edgeRing);
    
    // Secondary inner ring with different color for visual interest
    const innerRingGeometry = new THREE.TorusGeometry(30, 0.5, 20, 100);
    ...
    innerRing.position.y = 1.2; // Slightly above the outer ring
    hubGroup.add(innerRing);
    
    // Add decorative glowing circles to the platform
    const circlePatterns = [
        { radius: 25, count: 8, color: 0xff5555, size: 1.5, y: 1.3 },
        { radius: 15, count: 6, color: 0xff8800, size: 1.0, y: 1.4 }
    ];
    ...
    circlePatterns.forEach(pattern => {
        for (let i = 0; i < pattern.count; i++) {
            ...
            const z = Math.sin(angle) * pattern.radius;
            
            const circleGeometry = new THREE.CircleGeometry(pattern.size, 32);
            ...
            circle.position.set(x, pattern.y, z);
    ...
    // Main tower (dark with red glow and improved geometry)
    const towerHeight = 120; // Taller tower
    const towerGeometry = new THREE.CylinderGeometry(15, 22, towerHeight, 16, 8);
    ...
    tower.position.y = towerHeight/2 + 1; // Half height + platform height
    tower.castShadow = true;
    hubGroup.add(tower);
    
    // Add decorative horizontal rings around the tower
    const ringPositions = [0.2, 0.4, 0.6, 0.8]; // Position percentages up the tower
    
    ringPositions.forEach((pos, index) => {
        const ringRadius = 16 - (index * 0.8); // Decreasing size as they go up
        const ringGeo = new THREE.TorusGeometry(ringRadius, 0.8, 16, 64);
        ...
        ring.position.y = towerHeight * pos;
        hubGroup.add(ring);
    ...
    // Add vertical energy lines on the tower
    const towerLineCount = 8;
    for (let i = 0; i < towerLineCount; i++) {
        const angle = (i / towerLineCount) * Math.PI * 2;
        const x = Math.cos(angle) * 15.2; // Just outside tower radius
        const z = Math.sin(angle) * 15.2;
        
        const lineHeight = towerHeight * 0.8;
        const lineGeometry = new THREE.PlaneGeometry(1.5, lineHeight);
        const lineMaterial = new THREE.MeshStandardMaterial({
            color: 0xff3333,
            emissive: 0xff3333,
            emissiveIntensity: 2.0,
        ...
        const line = new THREE.Mesh(lineGeometry, lineMaterial);
        line.position.set(x, towerHeight/2, z);
        line.lookAt(new THREE.Vector3(0, towerHeight/2, 0)); // Orient toward center
    ...
    // Multiple X-MACHINA signs on different sides of the tower
    const signPositions = [
        { angle: 0, distanceFactor: 1.1 },    // Front
        { angle: Math.PI, distanceFactor: 1.1 },    // Back
        { angle: Math.PI/2, distanceFactor: 1.1 },  // Right
        { angle: -Math.PI/2, distanceFactor: 1.1 }  // Left
    ];
    
    signPositions.forEach((pos, index) => {
        // Modern floating neon sign panel with no visible backing
        const signWidth = 42;
        const signHeight = 12;
        
        // Thin frame around the sign for subtle definition
        const frameGeometry = new THREE.BoxGeometry(signWidth + 0.8, signHeight + 0.8, 0.2);
        ...
        // Position sign around the tower
        const towerRadius = 15;
        const distance = towerRadius * pos.distanceFactor;
        const x = Math.cos(pos.angle) * distance;
        const z = Math.sin(pos.angle) * distance;
        
        frame.position.set(x, towerHeight/2, z);
        frame.lookAt(new THREE.Vector3(0, towerHeight/2, 0)); // Face toward center
        frame.rotateY(Math.PI); // Flip to face outward
        
        hubGroup.add(frame);
        
        // Create X-MACHINA text with enhanced neon effect
        createHubText(frame, "X-MACHINA", 0xff0000, true); // Bright red text with enhanced neon
    });
    
    // Energy beam from top of tower with improved effects
    const beamHeight = 250; // Taller beam
    const beamGeometry = new THREE.CylinderGeometry(2, 6, beamHeight, 16, 8);
    ...
    beam.position.y = towerHeight + beamHeight/2; // Top of tower + half beam height
    hubGroup.add(beam);
    
    // Add spiral effect around the beam
    const spiralCount = 3;
    const spiralSegments = 100;
    const spiralRadius = 8;
    const spiralTurns = 8;
    const spiralHeight = beamHeight * 0.7;
    ...
    for (let s = 0; s < spiralCount; s++) {
        ...
        for (let i = 0; i <= spiralSegments; i++) {
            ...
            const angle = t * Math.PI * 2 * spiralTurns + (s * Math.PI * 2 / spiralCount);
            const radius = spiralRadius * (1 - 0.3 * t); // Decreasing radius
            
            const x = Math.cos(angle) * radius;
            const y = towerHeight + t * spiralHeight;
    ...
    // Add enhanced top sphere with better glow effect
    const topSphereGeometry = new THREE.SphereGeometry(12, 32, 32);
    ...
    topSphere.position.y = towerHeight + 1; // Top of tower
    hubGroup.add(topSphere);
    
    // Add glow effect to the top sphere
    const sphereGlowGeometry = new THREE.SphereGeometry(14, 32, 32);
    ...
    // Add enhanced energy lines from top sphere with more dynamic shapes
    const topLineCount = 16; // More lines
    const lineHeight = 100; // Longer lines
    
    for (let i = 0; i < topLineCount; i++) {
        const angle = (i / topLineCount) * Math.PI * 2;
        
        // Create tapered energy line
        const lineGeometry = new THREE.CylinderGeometry(0.2, 0.8, lineHeight, 8, 4);
        const lineMaterial = new THREE.MeshStandardMaterial({
            color: 0xff6666,
            roughness: 0.1,
            metalness: 0.9,
        ...
        line.position.y = towerHeight + lineHeight/2 + 15; // Top of sphere + half line height
        
        // Rotate the line to point outward and upward
        line.rotation.x = Math.PI / 6; // More vertical angle
        line.rotation.y = angle; // Rotate around y-axis
    ...
    // Add enhanced point lights with more intensity and range
    const redLight = new THREE.PointLight(0xff0000, 2, 150); // Brighter with more range
    redLight.position.set(0, towerHeight/2, 0);
    hubGroup.add(redLight);
    
    const topLight = new THREE.PointLight(0xffffff, 3, 300); // Brighter with more range
    topLight.position.set(0, towerHeight + 10, 0);
    hubGroup.add(topLight);
    
    // Add secondary lights for more dynamic lighting
    const baseLight = new THREE.PointLight(0xff6633, 1.5, 100);
    baseLight.position.set(0, 5, 0);
    hubGroup.add(baseLight);
    
    // Position the entire hub
    hubGroup.position.set(hubX, 0, hubZ);
    
    // Store hub info
    window.X_MACHINA_HUB = {
        mesh: hubGroup,
        position: { x: hubX, y: 0, z: hubZ },
        radius: 40, // Radius of the platform
        height: towerHeight + beamHeight,
        type: "AI_SYSTEM_CORE"
    };
    
    console.log("Created enhanced X-MACHINA HUB at western edge of X-axis");

    // Return the necessary elements for animation
    return { hubGroup, edgeRing, innerRing, topSphere, beamMaterial, sphereGlow };
}

// Create text for the X-MACHINA HUB sign with enhanced neon effect
function createHubText(signMesh, text, color, enhancedNeon = false) {
    const fontLoader = new FontLoader();
    ...
    fontLoader.load('https://threejs.org/examples/fonts/helvetiker_bold.typeface.json', function(font) {
        try {
            const textOptions = {
                font: font,
                size: 5.2, // Larger size for better visibility
                height: 0.4, // Thinner depth for more authentic neon look
                curveSegments: 32, // Higher detail for smoother curves
                bevelEnabled: true,
                bevelThickness: 0.2, // Smaller bevel for cleaner edges
                bevelSize: 0.15,
                bevelOffset: 0,
            ...
            // Create primary material for the text core - pure emissive for no shadows
            const textCoreMaterial = new THREE.MeshBasicMaterial({
                color: 0xffffff, // White core
                emissive: 0xffffff,
                emissiveIntensity: 1.0
            });
            
            // Create the text core mesh
            const textCoreMesh = new THREE.Mesh(textGeometry, textCoreMaterial);
            textCoreMesh.position.set(0, 0, 1.2); // Slightly in front of the sign
            
            // Add the text core to the sign
            signMesh.add(textCoreMesh);
            
            // Create the colored glow layer that gives it the neon color
            const coloredGlowMaterial = new THREE.MeshBasicMaterial({
                color: color,
                transparent: true,
                opacity: 0.9
            });
            
            const coloredGlowGeometry = textGeometry.clone();
            const coloredGlowMesh = new THREE.Mesh(coloredGlowGeometry, coloredGlowMaterial);
            coloredGlowMesh.scale.set(1.05, 1.05, 1.05);
            coloredGlowMesh.position.copy(textCoreMesh.position);
            signMesh.add(coloredGlowMesh);
            
            // Multi-layered glow effect for enhanced neon signs - using BasicMaterial to avoid shadows
            const glowLayers = [
                { size: 0.15, opacity: 0.9, color: color },
                { size: 0.3, opacity: 0.7, color: color },
                { size: 0.6, opacity: 0.5, color: color },
                { size: 1.2, opacity: 0.3, color: color },
                { size: 1.8, opacity: 0.15, color: 0xffffff } // Outer white halo
            ];
            ...
            glowLayers.forEach(layer => {
                const glowMaterial = new THREE.MeshBasicMaterial({
                    ...
                    side: THREE.BackSide,
                    blending: THREE.AdditiveBlending // Additive blending for better glow effect
                });
                
                const glowGeometry = textGeometry.clone();
                const glowMesh = new THREE.Mesh(glowGeometry, glowMaterial);
                glowMesh.scale.set(1 + layer.size, 1 + layer.size, 1 + layer.size);
                glowMesh.position.copy(textCoreMesh.position);
                
                signMesh.add(glowMesh);
            });
            
            // Add volumetric light cone effect behind the text
            ...
            const coneLength = 20;
            const coneGeometry = new THREE.ConeGeometry(signHeight * 0.7, coneLength, 32, 1, true);
            const cone = new THREE.Mesh(coneGeometry, coneMaterial);
            
            // Position and rotate cone to project from the text
            cone.position.set(0, 0, -coneLength/2 + 1);
            cone.rotation.x = Math.PI / 2;
            textCoreMesh.add(cone);
            
            // Add powerful point lights for each letter to create glow in the environment
            const letterSpacing = signWidth / (text.length + 1);
            const startX = -signWidth/2 + letterSpacing;
            
            for (let i = 0; i < text.length; i++) {
                const letterLight = new THREE.PointLight(color, 1.0, 15);
                letterLight.position.set(startX + i * letterSpacing, 0, 2);
                
                // Store animation data
                letterLight.userData = {
                    originalIntensity: 1.0,
                    flickerSpeed: 0.2 + Math.random() * 0.3,
                    flickerIntensity: 0.3
                };
                
                signMesh.add(letterLight);
            }
            
            // Add flicker effect data for animation
            textCoreMesh.userData = {
                originalIntensity: 1.0,
                flickerSpeed: 0.15 + Math.random() * 0.2,
                flickerIntensity: 0.15
            };
            
            coloredGlowMesh.userData = {
                originalOpacity: 0.9,
                flickerSpeed: 0.15 + Math.random() * 0.2,
                flickerIntensity: 0.15
            };
            ...
            console.error("Error creating X-MACHINA text:", error);
        }
    });
}
...
function animateXMachinaHub(hubGroup, edgeRing, innerRing, topSphere, beamMaterial, sphereGlow) {
    ...
    function animateHub() {
        ...
        hubGroup.children.forEach(child => {
            // Animate energy lines from top
            if (child.userData && child.userData.angle !== undefined) {
                // Vertical movement with more dramatic motion
                child.position.y = child.userData.originalY + Math.sin(time * child.userData.speed * 5) * 8;
                
                // Pulse opacity
                child.material.opacity = 0.5 + 0.5 * Math.sin(time * child.userData.speed + child.userData.angle);
                
                // Add slight rotation for more dynamic effect
                child.rotation.z = Math.sin(time * child.userData.speed * 0.5) * 0.1;
            }
            
            // Animate platform circles
            if (child.userData && child.userData.pulseSpeed !== undefined && child.userData.originalIntensity !== undefined) {
                child.material.emissiveIntensity = child.userData.originalIntensity * (0.7 + 0.5 * Math.sin(time * child.userData.pulseSpeed));
            }
            ...
            // Animate spirals
            if (child.type === 'Line' && child.userData.originalOpacity !== undefined) {
                child.material.opacity = child.userData.originalOpacity * (0.6 + 0.4 * Math.sin(time * child.userData.pulseSpeed));
            }
        });
...
// Check if player is near the cyborg girl
function checkCyborgGirlProximity() {
    if (!window.cyborgGirl || !playerShip) return;
    
    const distanceThreshold = 30; // Distance at which to react
    const position = playerShip.position;
    const cyborgPosition = window.cyborgGirl.position;
    
    // Calculate distance
    const dx = position.x - cyborgPosition.x;
    const dz = position.z - cyborgPosition.z;
    const distance = Math.sqrt(dx * dx + dz * dz);
    
    // When player gets close
    if (distance < distanceThreshold) {
        // Make her look toward the player
        const angle = Math.atan2(dx, dz);
        window.cyborgGirl.rotation.y = angle;
        
        // Make her do the walking animation if not already
        if (!window.cyborgGirl.userData.isWalking && distance < 20) {
            window.cyborgGirl.userData.isWalking = true;
            
            // If really close, show a message (only once)
            if (distance < 10 && !window.cyborgGirlIntroduced) {
                window.cyborgGirlIntroduced = true;
                showMessage("CYBORG INTERFACE: Welcome to Aurora Prime, I am the maintenance unit X-501. The AI hub is accessible through the western corridor.", 5000);
            }
        }
    } else {
        // Return to idle animation if player moves away
        if (window.cyborgGirl.userData.isWalking) {
            window.cyborgGirl.userData.isWalking = false;
        }
    }
}
...
function showMessage(text, duration = 3000) {
    ...
    if (!messageEl) {
        ...
        messageEl.style.boxShadow = '0 0 10px #00ffff';
        messageEl.style.borderTop = '1px solid #00ffff';
        messageEl.style.borderBottom = '1px solid #00ffff';
