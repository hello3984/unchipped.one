...
async function initGame() {
    try {
        // Initialize THREE.js and core components
        const loaded = await ModuleLoader.loadModules();
        if (!loaded) {
            throw new Error("Failed to load required modules");
        }
        ...
        // Set up post-processing if available and not on mobile
        if (ModuleLoader.isPostProcessingAvailable() && !isMobileDevice()) {
        ...
        if (isMobileDevice()) {
            // Mobile controls already initialized by TouchController
